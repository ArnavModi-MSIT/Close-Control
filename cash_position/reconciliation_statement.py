...
from . import config
from .engine import classify_positions


class ReconciliationInvariantError(Exception):
    """Raised when the bank-side five-bucket partition doesn't actually
    ...
    complement of the other four masks, so the partition holds by
...
def _mixed_settlement_adjustment(detail: pd.DataFrame, confirmed_settlement_ids: set) -> dict:
    ...
    non_confirmed_in_mixed = detail[
        detail["settlement_id"].isin(confirmed_settlement_ids) & (detail["cash_bucket"] != "confirmed")
    ]
    return {
...
def _confirmed_bucket_totals(detail: pd.DataFrame) -> dict:
    confirmed = detail[detail["cash_bucket"] == "confirmed"]
    return {
...
def _bank_side_coverage(gateway: pd.DataFrame, bank: pd.DataFrame,
    ...
    of five buckets, verified to partition the full bank statement with no
    ...
    unexplained_mask = ~(matched_confirmed_mask | matched_other_mask | ambiguous_mask | orphan_mask)
    ...
    if unexplained_count:
        raise ReconciliationInvariantError(
            ...
            f"all five buckets -- the partition invariant that should hold by construction has "
...
def build_reconciliation_statement(report: pd.DataFrame, gateway: pd.DataFrame, bank: pd.DataFrame,
    ...
    captured = detail[detail["cash_bucket"] != "not_yet_captured"]

    books_ending_balance = float(captured["ledger_expected_net_rupees"].sum())

    in_transit = detail[detail["cash_bucket"] == "in_transit"]
    held = detail[(detail["cash_bucket"] == "at_risk") & detail["is_held"]]
    at_risk_other = detail[(detail["cash_bucket"] == "at_risk") & ~detail["is_held"]]
