...
Allowlisted files, and why each is legitimate:
  - scripts/evaluate.py                  the one sanctioned reader (this
                                          project's own stated rule)
  - scripts/run_baseline_naive.py        delegates to evaluate.py's own
                                          load_ground_truth() -- the SAME
                                          reader, reused, not a second one
  - scripts/generate_data.py             the WRITER -- produces
  ...
  - data_generation/ground_truth.py      builds the ground-truth DataFrame
    ...
    python tests/test_ground_truth_isolation.py
"""
...
ALLOWLISTED_FILES = {
    ...
    os.path.join("scripts", "run_baseline_naive.py"),
    ...
    os.path.join("scripts", "run_cash_position.py"),
}
...
def main() -> None:
    ...
    # Compared against the full relative path, not os.path.basename(__file__)
    # -- this file itself now lives at tests/test_ground_truth_isolation.py,
    # not the repo root, so a bare-basename comparison against rel_path
    # (which includes the "tests/" prefix) would never match, and this
    # file would incorrectly scan and flag ITSELF for mentioning
    # "ground_truth.csv" in its own docstring/allowlist comments above.
    self_rel_path = os.path.relpath(os.path.abspath(__file__), REPO_ROOT)
    violations = []
    for rel_path in py_files:
        if rel_path == self_rel_path or rel_path in ALLOWLISTED_FILES:
            continue
