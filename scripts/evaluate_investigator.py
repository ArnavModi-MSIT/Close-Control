...
what agent/gate.py itself checks at decision time.
    ...
    python scripts/evaluate_investigator.py
    python scripts/evaluate_investigator.py --n-per-type 3
"""
...
def print_report(scored: pd.DataFrame, missing: list, target_n: int) -> None:
    ...
    if missing:
        ...
        for txn_id in missing:
            print(f"  python scripts/run_investigator.py --transaction-id {txn_id}")
    if scored.empty:
